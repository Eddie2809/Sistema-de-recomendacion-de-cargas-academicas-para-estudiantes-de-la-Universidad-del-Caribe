...
class Algoritmo():
	def __init__(self,*args,situacion, datosEntrenamientoModelo, datosEntrenamientoKM, datosCeneval, tasasReprobacion, matricula, NGEN,obtenerCancelarEjecucion,setCancelarEjecucion,kardex,planNombre,periodoActual,pesos,disponibilidad,cantidadIdealMaterias,disponibilidadComoRestriccion,**kwargs):
	...
	def obtenerTotalRecursando(self, solucion):
		# Se desnormaliza el valor devuleto por UpMR y se obtiene la cantidad de materias reprobadas de una carga
		return self.UpMR(solucion) * self.materiasReprobadasOfertadas

	def obtenerComplejidadCarga5(self, solucion):
	...
	def obtenerTasaRepCarga(self, solucion):
	...
	def CpRR(self, solucion):
		entrada = [[self.obtenerTasaRepCarga(solucion), self.ceneval_analitico, self.ceneval_matematico, self.ceneval_lengua, self.ceneval_esp, self.tasa_aprob_per_prev, self.carrera_IA, self.carrera_IDeIO, self.carrera_NI, self.carrera_TS, self.semestre_Otoño, self.obtenerComplejidadCarga5(solucion), self.año_encurso, self.obtenerTotalRecursando(solucion), self.situacion_Condicionado, self.situacion_Irregular]]
